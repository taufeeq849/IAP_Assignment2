...
int main()
{
    ...
    // vector<string> cards = {"7s", "5s", "3h"};
    // string dealercard = "As";
...
string getAdvice(const vector<string> &playerHand, string dealerCard)
{
    ...
    string ret;
    int playervalue = getHighScore(playerHand);
    bool hasAce = false;
    for (string temp : playerHand)
    {
        if (temp[0] == 'A')
        {
            hasAce = true;
...
int getHighScore(const vector<string> &hand)
{
    int sum =  getPlayerScore(hand).first;
if (getPlayerScore(hand).second && (sum + 10) <=21 ) {
